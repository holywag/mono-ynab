...
class YnabApiWrapper:
    ...
    def bulk_create_transactions(self, budget_id, transactions: list[YnabTransactionFields]):
        transactions_api = ynab.TransactionsApi(self.__client)
        response = transactions_api.bulk_create_transactions(
            budget_id, ynab.BulkTransactions(list(map(lambda t: ynab.SaveTransaction(**asdict(t)), transactions))))
...
class SingleBudgetYnabApiWrapper:
    ...
    def bulk_create_transactions(self, transactions: list[YnabTransactionFields]):
